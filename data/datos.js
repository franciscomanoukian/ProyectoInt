let datos = {
    usuario: {
        nombreUsuario: DelfiAlric,
        clave: Programacion,
        recordarme: true // Va o no va??????
    },
    productos: [
        {
            imagen: '../public/images/products/nikejumbo.jpeg',
            nombre: 'Nike Blazer Jumbo',
            descripcion: "Alabadas por muchos por su aspecto y su sensación de resistencia, el básico de armario se actualiza con las Nike Blazer Mid ' 77 Jumbo.Aprovecha el look clásico que más te gusta, que ahora cuenta con un talón elástico con textura de pana y unas presillas grandes para que puedas ponértelas y quitártelas fácilmente.El diseño Swoosh extragrande y los cordones enormes añaden un toque divertido"
            , 
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/converse.jpeg',
            nombre: 'Converse Chuck 70s',
            descripcion: "Para un estilo en el que puedas confiar, las zapatillas Chuck Taylor All Star son un elemento básico de la moda. Las icónicas siluetas de caña alta y baja se mantienen fieles al legado. Esta es la opción que combina con todo, que no te vas a cansar",
            fechaCarga: '01-04-2023',
        },
        {
            imagen: '../public/images/products/goldengoose.jpeg',
            nombre: 'Golden Goose Superstar',
            descripcion: "Las zapatillas 'Superstar' de Golden Goose han sido cepilladas y enceradas a mano para darles la apariencia y la sensación de unos viejos favoritos. Confeccionadas en cuero suave, este par blanco está decorado con la aplicación de estrella característica de la marca. Úsalas durante el fin de semana con una camiseta y unos jeans.",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/nikejordans.jpeg',
            nombre: 'Nike Jordans mid',
            descripcion: "Inspiradas en las AJ1 originales, las Air Jordan 1 Mid ofrecen a los aficionados la oportunidad de seguir los pasos de MJ. El nuevo color ribetea los materiales clásicos e impecables",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/veja.jpeg',
            nombre: 'Veja Esplar Low',
            descripcion: "Diseñado en una horma de zapato unisex utilizando tallas europeas, forro técnico hecho de botellas de plástico 100% recicladas. Un estilo minimalista y de líneas limpias, estas zapatillas Veja son un par que combina con todo",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/nikeblazerlow.jpeg',
            nombre: 'Nike Blazer Low',
            descripcion: "Inspiradas en el look clásico de baloncesto, las zapatillas Nike Blazer Low cuentan con una parte superior de piel y una zona del tobillo de corte bajo para ofrecer comodidad y durabilidad. Confección en autoclave que une la suela exterior con la mediasuela. Para ir al trabajo o disfrutar del fin de semana, luce un look estilizado para todos los días de la semana.",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/zadigwhite.jpeg',
            nombre: 'Z&V High Flash White',
            descripcion: "Icónicas y esenciales, las zapatillas altas ZV1747 High Flash están disponibles en piel blanca tono sobre tono. Plantilla cuña de 2,5 cm.Este modelo calza normal, te recomendamos que cojas tu talla habitua",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/zadigcanvas.jpeg',
            nombre: 'Z&V High Flash Canvas',
            descripcion: "Las zapatillas de alta gama ZV1747 High Flash están disponibles esta temporada en lona de algodón. Presentan bordes crudos, destellos pespunteados, cordones contrastantes y cierre de cremallera.",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/offwhite.jpeg',
            nombre: 'Off White Orange',
            descripcion: "Zapatillas Off-Court 3.0 de caña alta en blanco con el logotipo de Off-White™ en un lado y un parche de flecha tonal en el otro lado. Detalles de etiquetas naranjas. Suela de goma blanca. Cierre de cordones blancos.",
            fechaCarga: '01-04-2023'
        },
        {
            imagen: '../public/images/products/adidassamba.jpeg',
            nombre: 'Adidas Samba',
            descripcion: "Inspiradas en los colores de Argentina, estas zapatillas adidas Samba Team combinan las 3 tiras blancas con una parte superior de cuero sintético verde bosque. El escudo del equipo en la lengüeta muestra el orgullo que sentís cada vez que te las ponés. Además, si las das vuelta vas a ver la clásica suela exterior de caucho desarrollada por primera vez para el entrenamiento de fútbol en 1949.",
            fechaCarga: '01-04-2023'
        }
    ],
    comentarios: [
        {
            nombreUsuario: 'FranManoukian',
            textoComentario: 'FAcheras',
            imagenPerfil: null
        },
        {
            nombreUsuario: 'JustiAlric',
            textoComentario: 'Altas zapas!',
            imagenPerfil: null
        },
    ]
}

module.exports = datos